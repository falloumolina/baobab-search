...
const LANGUAGES = {
  "fr-FR": {
    name: "Français",
    search: "Recherche de", resultsFor: "Résultats pour", imagesFor: "Images pour", videosFor: "Vidéos pour",
    newsFor: "Actualités pour", mapsFor: "Maps pour", baobabIA: "Baobab IA",
    seeResults: "Voir les résultats sur:", all: "Tous", images: "Images", videos: "Vidéos", news: "Actualités", maps: "Maps",
    micNotSupported: "Micro non supporté", press: "Presse", video: "Vidéo", newsItem: "Actualité", hoursAgo: "Il y a",
    localization: "Localisation de", onMap: "sur la carte.", summary: "Voici les résultats trouvés pour",
    imagesText: "Voici", inImages: "en images.", videosText: "Voici les meilleures vidéos sur",
    newsText: "Voici les dernières actualités sur"
  },
  "en-US": {
    name: "English", search: "Searching for", resultsFor: "Results for", imagesFor: "Images for", videosFor: "Videos for",
    newsFor: "News for", mapsFor: "Maps for", baobabIA: "Baobab AI", seeResults: "See results on:",
    all: "All", images: "Images", videos: "Videos", news: "News", maps: "Maps", micNotSupported: "Microphone not supported",
    press: "Press", video: "Video", newsItem: "News", hoursAgo: "h ago", localization: "Location of", onMap: "on the map.",
    summary: "Here are the results found for", imagesText: "Here are", inImages: "in images.", videosText: "Here are the best videos about",
    newsText: "Here are the latest news about"
  },
  "ar-SA": {
    name: "العربية", search: "البحث عن", resultsFor: "النتائج لـ", imagesFor: "صور لـ", videosFor: "فيديوهات لـ",
    newsFor: "أخبار لـ", mapsFor: "الخرائط لـ", baobabIA: "باوباب الذكاء الاصطناعي", seeResults: "عرض النتائج على:",
    all: "الكل", images: "صور", videos: "فيديو", news: "أخبار", maps: "خرائط", micNotSupported: "الميكروفون غير مدعوم",
    press: "صحافة", video: "فيديو", newsItem: "خبر", hoursAgo: "منذ", localization: "موقع", onMap: "على الخريطة.",
    summary: "فيما يلي النتائج التي تم العثور عليها لـ", imagesText: "إليك", inImages: "بالصور.", videosText: "إليك أفضل الفيديوهات حول",
    newsText: "إليك آخر الأخبار حول"
  },
  "es-ES": {
    name: "Español", search: "Buscando", resultsFor: "Resultados para", imagesFor: "Imágenes de", videosFor: "Videos de",
    newsFor: "Noticias sobre", mapsFor: "Mapas de", baobabIA: "Baobab IA", seeResults: "Ver resultados en:",
    all: "Todo", images: "Imágenes", videos: "Videos", news: "Noticias", maps: "Mapas", micNotSupported: "Micrófono no compatible",
    press: "Prensa", video: "Video", newsItem: "Noticia", hoursAgo: "hace", localization: "Ubicación de", onMap: "en el mapa.",
    summary: "Aquí están los resultados encontrados para", imagesText: "Aquí tienes", inImages: "en imágenes.", videosText: "Aquí están los mejores videos sobre",
    newsText: "Aquí están las últimas noticias sobre"
  },
  "wo-SN": {
    name: "Wolof", search: "Di seet", resultsFor: "Résultat yi ci", imagesFor: "Natal yi ci", videosFor: "Vidéo yi ci",
    newsFor: "Lëndëm yi ci", mapsFor: "Carte bi ci", baobabIA: "Baobab AI", seeResults: "Gisal résultat yi ci:",
    all: "Lépp", images: "Natal", videos: "Vidéo", news: "Lëndëm", maps: "Carte", micNotSupported: "Micro bi du dara",
    press: "Presse", video: "Vidéo", newsItem: "Lëndëm", hoursAgo: "ci", localization: "Benn bi", onMap: "ci carte bi.",
    summary: "Lépp lu ma gis ci", imagesText: "Lépp", inImages: "ci natal.", videosText: "Lépp vidéo yu baax ci",
    newsText: "Lépp lëndëm yu bees ci"
  },
  "pt-PT": {
    name: "Português", search: "Pesquisando por", resultsFor: "Resultados para", imagesFor: "Imagens de", videosFor: "Vídeos de",
    newsFor: "Notícias sobre", mapsFor: "Mapas de", baobabIA: "Baobab IA", seeResults: "Ver resultados em:",
    all: "Tudo", images: "Imagens", videos: "Vídeos", news: "Notícias", maps: "Mapas", micNotSupported: "Microfone não suportado",
    press: "Imprensa", video: "Vídeo", newsItem: "Notícia", hoursAgo: "há", localization: "Localização de", onMap: "no mapa.",
    summary: "Aqui estão os resultados encontrados para", imagesText: "Aqui estão", inImages: "em imagens.", videosText: "Aqui estão os melhores vídeos sobre",
    newsText: "Aqui estão as últimas notícias sobre"
  }
};

function t(key){
  return LANGUAGES[lang]?.[key] || LANGUAGES["fr-FR"][key];
}
...
function showPage(id){
  ...
  window.scrollTo(0,0);
  applyLanguage(); // Applique la langue à chaque changement de page
}
...
function applyLanguage(){
  ...
  document.querySelectorAll('.filter-btn').forEach((b,i)=>{
    if(filters[i]) b.textContent = filters[i];
  });
}
...
function setActiveFilter(f){
  ...
  document.querySelectorAll('.filter-btn').forEach(b=>{
    if(b.textContent.toLowerCase().includes(txt)){
...
async function searchWeb(query){
  ...
  try {
    ...
    iaHtml = `<div style="background:var(--card);border-left:4px solid var(--accent);padding:16px;margin:16px 24px;border-radius:8px">
      <div style="font-weight:700;color:var(--accent);margin-bottom:8px">${t('baobabIA')}</div>
  ...
  if(resultsHtml === ""){
    resultsHtml = `<div style="padding:20px;text-align:center">
      <p style="margin-bottom:12px">${t('seeResults')}</p>
...
async function searchImages(query){
  let iaHtml = `<div style="background:var(--card);border-left:4px solid var(--accent);padding:16px;margin:16px 24px;border-radius:8px">
    <div style="font-weight:700;color:var(--accent);margin-bottom:8px">${t('baobabIA')}</div>
    <div>${t('imagesText')} ${query} ${t('inImages')}</div>
  </div>`;
  ...
  $('#resultsList').innerHTML = `<p style="padding:12px 24px">${t('imagesFor')} <b>${query}</b></p>` + iaHtml +
...
async function searchVideos(query){
  ...
  for(let i=1; i<=12; i++){
    list += `<div style="padding:14px 24px;border-bottom:1px solid var(--border);cursor:pointer" onclick="window.open('https://www.youtube.com/results?search_query=${encodeURIComponent(query)}','_blank')">
      <div style="font-size:18px;color:var(--link)">🎬 ${query} - ${t('video')} ${i}</div>
    ...
    if(newsRes.ok){
      ...
      if(news.results) news.results.slice(0,12).forEach(item=>{
        list += `<div style="padding:14px 24px;border-bottom:1px solid var(--border);cursor:pointer" onclick="window.open('${item.link}','_blank')">
          ...
          <div style="color:var(--muted);font-size:14px">${new Date(item.pubDate).toLocaleDateString(lang)} - ${item.source_id || t('press')}</div>
        </div>`;
      });
    }
  }catch(e){}
  ...
  if(list===""){
    for(let i=1; i<=12; i++){
      list += `<div style="padding:14px 24px;border-bottom:1px solid var(--border);cursor:pointer" onclick="window.open('https://news.google.com/search?q=${encodeURIComponent(query)}&hl=fr&gl=FR','_blank')">
        <div style="font-size:18px;color:var(--link)">📰 ${query} - ${t('newsItem')} ${i}</div>
        <div style="color:var(--muted);font-size:14px">${t('hoursAgo')} ${i}h - Google News</div>
      </div>`;
    }
  }
...
function searchMaps(query){
  let iaHtml = `<div style="background:var(--card);border-left:4px solid var(--accent);padding:16px;margin:16px 24px;border-radius:8px">
    <div style="font-weight:700;color:var(--accent);margin-bottom:8px">${t('baobabIA')}</div>
    <div>${t('localization')} "${query}" ${t('onMap')}</div>
  </div>`;

  $('#resultsList').innerHTML = `<p style="padding:12px 24px">${t('mapsFor')} <b>${query}</b></p>` + iaHtml +
...
function startVoice(){
  ...
  if(!SR) return alert(t('micNotSupported'));
